from hera.workflows import DAG, Resources, Workflow, WorkflowsService, script


@script(
    image="jupyter/pyspark-notebook:latest",
    resources=Resources(cpu_request=4, memory_request="8Gi"),
)
def spark(num_data: int) -> None:
    ...
    spark = (
        ...
        .appName("my-spark-example-running-in-hera.com")
        .getOrCreate()
    )
    ...
    print(pandas_result)
    print(
        "Pandas dataframe took {pandas_elapsed} seconds to compute".format(
            pandas_elapsed=pandas_elapsed
        )
    )
    ...
    print(spark_result)
    print(
        "Spark dataframe took {spark_elapsed} seconds to compute".format(
